...
export default function PDFCanvas({
  ...
  const { currentPage, zoom, currentTool, selectedShapeType, elements } =
    ...
    elements.forEach((element, index) => {
      ...
      if (canvasObj) {
        ...
        if (
          ...
          (canvasObj as fabric.Object).angle !== element.rotation
        ) {
        ...
        if (
          ...
          (canvasObj as fabric.Object).opacity !== element.opacity
        ) {
        ...
        if (element.type === "shape") {
          ...
          // 도형 색상 업데이트
          if (
            (canvasObj as fabric.Object).stroke !== shapeElement.strokeColor
          ) {
            (canvasObj as fabric.Object).set(
              "stroke",
          ...
          // 선 굵기 업데이트
          if (
            (canvasObj as fabric.Object).strokeWidth !==
  ...
  const restoreElementsToCanvas = (canvas: fabric.Canvas) => {
    ...
    console.log(
      "ElementManager 전체 요소 개수:",
      elementManager.getAllElements().length,
    );
        ...
        if (shapeElement.shapeType === "rectangle") {
          ...
          shapeObj = new fabric.Line(
            [
              ...
              shapeElement.y + shapeElement.height,
            ],
            {
              stroke: shapeElement.strokeColor,
              strokeWidth: shapeElement.strokeWidth,
  ...
  const setupEventListeners = (canvas: fabric.Canvas) => {
    ...
    canvas.on("selection:created", (e) => {
      const selected =
        e.selected?.map((obj: any) => obj.id).filter(Boolean) || [];
      ...
      if (
        ...
        if (selectedShapeTypeRef.current === "rectangle") {
          ...
          shape = new fabric.Line(
            [
              shapeStartPointRef.current.x,
              shapeStartPointRef.current.y,
              e.pointer.x,
              e.pointer.y,
            ],
            {
    ...
    canvas.on("mouse:up", (e) => {
      ...
      if (
        ...
        const minSize = 10;
        const sizeValid =
          selectedShapeTypeRef.current === "line"
